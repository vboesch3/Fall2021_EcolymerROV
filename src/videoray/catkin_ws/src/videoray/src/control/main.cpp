...
//std_msgs::Float32 star;
//std_msgs::Float32 port;
int star_thrust_;
int port_thrust_;
int vert_thrust_;
...
void callback_throttle( const videoray::ThrottlePtr& msg)
{
	
	throttle_ = *msg;
	port_thrust_ =  msg->PortInput;
	star_thrust_ =  msg->StarInput;
	vert_thrust_ = msg->VertInput;
	cout << "Vertical thrust callback" << vert_thrust_ << endl;
...
{     
     ...
     ros::Subscriber throttle_sub_ = n_.subscribe<>("throttle", 1, callback_throttle);
     ...
     while (ros::ok()) {                   
          ...
          if (joystick_enabled_) {
               ...
               port_thrust_ -= turn_;
               star_thrust_ += turn_;
		//cout<< "Port: " << port << " Star: " << star<< " Current Depth: " << comm.depth() << endl;
               //cout<< "Port: " << throttle_.PortInput << " Star: " << throttle_.StarInput<< endl;
		if( (abs(port_thrust_) - 20) < 19){
			port_thrust_ = throttle_.PortInput;
		}
		if( (abs(star_thrust_) - 20) < 19){
			star_thrust_ = throttle_.StarInput;
		}
               ...
               if (button_[10]) { // Open manipulator     
                    manip_state_ = VideoRayComm::Opening;
		std::cout << "Entering manip state" << std::endl;
               } else if (button_[11]) { // Close manipulator
	  ...
	  status = comm.set_focus(focus_);          
	  cout << "Vertical thruster" << vert_thrust_ <<endl;
