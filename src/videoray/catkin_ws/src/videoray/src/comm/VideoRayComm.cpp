...
VideoRayComm::Status_t VideoRayComm::set_desired_depth(int depth)
{
     if (depth < 0) {
          ...
          tx_ctrl_data[AUTO_DEPTH_LSB] = (depth & 0x00FF) ; // depth & 0x00FF;
          tx_ctrl_data[AUTO_DEPTH_MSB] = (depth & 0xFF00) >> 8; // (depth & 0xFF00) >> 8;
     }
     return VideoRayComm::Success;
}
...
VideoRayComm::Status_t VideoRayComm::send_nav_data_command()
{
     ...
     if (status == Packetizer::Success) {
          ...
          depth_ = (swap_bytes(packet, DEPTH_MSB, DEPTH_LSB) / 100.0) - 9.8; //was /10
